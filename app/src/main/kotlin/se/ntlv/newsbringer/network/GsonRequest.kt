...
(
    ...
    override fun deliverResponse(response: T) {
        if (response != null) {
            ...
            crashlyticsLogIfPossible(Log.ERROR, TAG, "Request $this resulted in null response.")
        }
    }

    override fun parseNetworkResponse(response: NetworkResponse): Response<T> {
        try {
            val json = String(response.data, response.charset)
    ...
    val NetworkResponse.charset: Charset
        get() = charset(parseCharset(headers))
}
